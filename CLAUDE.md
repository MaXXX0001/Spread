...
- Delete-blocking pattern: DB `restrictOnDelete` + `DeleteAction->before()` notification and
  ...
  Exception: `clicks.campaign_id` has no FK (a click still in the stream must never fail its
  batch), so only the admin blocks deleting a campaign with clicks.
...
- Remote `origin` = github.com/MaXXX0001/Spread; the user allowed pushing flow branches and `main`.
