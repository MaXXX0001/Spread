...
- **Дублі:** після `INSERT` пачки один `UPDATE`:
  `UPDATE clicks c SET is_duplicate = true WHERE NOT c.is_duplicate AND c.ip IN (<ip пачки>) AND c.clicked_at BETWEEN <min ts пачки> AND <max ts пачки> + 60 s AND EXISTS (SELECT 1 FROM clicks p WHERE p.ip = c.ip AND p.user_agent IS NOT DISTINCT FROM c.user_agent AND p.clicked_at >= c.clicked_at - interval '60 seconds' AND p.clicked_at <= c.clicked_at AND (p.clicked_at, p.click_id) < (c.clicked_at, c.click_id))`.
  «Раніший клік» — старіший `clicked_at`, а в ту саму мілісекунду — менший `click_id` (порівняння пари `(clicked_at, click_id)`). Діапазон `+ 60 s` вгору дозволяє позначити пізніший клік, який уже в БД, коли раніший дійшов із запізненням (після `XAUTOCLAIM`), — тому результат не залежить від порядку обробки. Вікно — константа `DUPLICATE_WINDOW_SECONDS = 60` (підтверджено користувачем). Кліки без IP не позначаються. Вікно ковзне: серія кліків кожні 50 с позначає всі, крім першого.
